...
from src.main.reader.options.reader_options import CSV_OPTIONS, JSON_OPTIONS, EXCEL_OPTIONS
from src.main.reader.reader import Reader
from src.main.reader.schema.source_schema import ORDERS_SCHEMA, PRODUCTS_SCHEMA, CUSTOMERS_SCHEMA
...
class TestReader(unittest.TestCase):
    ...
    @patch('src.main.reader.reader.Reader.read_excel')
    def test_read(self, mock_read_excel, mock_read_file, ):
        """
        Test Reads the content of the file
        :return:
        """
        mock_read_file.return_value = self.mock_dataframe
        mock_read_excel.return_value = self.mock_dataframe

        test_scenario = [
            ("csv", "Products.csv", PRODUCTS_SCHEMA, CSV_OPTIONS),
            ("json", "Orders.json", ORDERS_SCHEMA, JSON_OPTIONS),
            ("excel", "Customers.xlsx", CUSTOMERS_SCHEMA, EXCEL_OPTIONS)
        ]
    ...
    @patch('src.main.reader.reader.pandas.read_excel')
    def read_excel(self, mock_read_excel):
        """
        Test Read Excel File with Specified Schema
        :return:
        """
        mock_pandas_dataframe=MagicMock()
        mock_read_excel.return_value = mock_pandas_dataframe
        mock_excel_schema = MagicMock()
        mock_load = MagicMock(spec=DataFrame)
        actual_df = self.reader_instance.read_excel(
            input_file_path=self.mock_file_path,
            input_schema=mock_excel_schema
        )
        mock_read_excel.assert_called_once_with(
            sheet_name=EXCEL_OPTIONS['sheetName'], io=self.mock_file_path
        )
        self.mock_spark.createDataFrame.assert_called_once_with(
            mock_pandas_dataframe,
            schema=mock_excel_schema
        )
        self.assertEqual(actual_df, mock_load)
